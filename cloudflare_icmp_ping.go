...
func Ping(addr string) (*net.IPAddr, time.Duration, float64, error) {
	...
	case ipv4.ICMPTypeEchoReply:
		...
		return dst, duration, packetloss, nil
	default:
		return dst, 0, 0, fmt.Errorf("got %+v from %v; want echo reply", rm, peer)
	}
}

func main() {
	p := func(addr string) {
		dst, dur, packetloss, err := Ping(addr)
		if err != nil {
			log.Printf("Ping %s (%s): %s\n", addr, dst, err)
			return
		}
		log.Printf("Ping %s (%s): %s %v%% packet loss\n", addr, dst, dur, packetloss)
	}
	p("127.0.0.1")
	p("172.27.0.1")
	p("google.com")
